...
static void H_K_R_Adjustment(KkalmanFilter_t *kf);
 ...
 * @param zSize 观测量维度
 */
void Kalman_Filter_Init(KkalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
...
void Kalman_Filter_Measure(KkalmanFilter_t *kf)
{
    // 矩阵H K R根据量测情况自动调整
...
void Kalman_Filter_xhatMinusUpdate(KkalmanFilter_t *kf)
{
    if (!kf->SkipEq1)
    {
        if (kf->uSize > 0)
...
void Kalman_Filter_PminusUpdate(KkalmanFilter_t *kf)
{
    if (!kf->SkipEq2)
    {
...
void Kalman_Filter_SetK(KkalmanFilter_t *kf)
{
    if (!kf->SkipEq3)
    {
...
void Kalman_Filter_xhatUpdate(KkalmanFilter_t *kf)
{
    if (!kf->SkipEq4)
    {
...
void Kalman_Filter_P_Update(KkalmanFilter_t *kf)
{
    if (!kf->SkipEq5)
    {
 ...
 * @return float* 返回滤波值
 */
float *Kalman_Filter_Update(KkalmanFilter_t *kf)
{
    // 0. 获取量测信息
