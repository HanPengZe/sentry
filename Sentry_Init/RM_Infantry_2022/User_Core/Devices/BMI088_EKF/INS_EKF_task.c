...
static const float imu_temp_PID[3] = {1600.0f, 0.2f, 0};
...
void INS_EKF_Init(void)
{
    ...
    PID_init(&imu_temp_pid, 0, imu_temp_PID, 4500.0f, 4400.0f);
...
static int float_rounding(float raw)
{
    static int integer;
...
void IMU_Temperature_Ctrl(void)
{
    ...
    PID_calc(&imu_temp_pid, BMI088.Temperature, RefTemp);

    // TIM_Set_PWM(&htim10, TIM_CHANNEL_1, float_constrain(float_rounding(imu_temp_pid.out), 0, UINT32_MAX));

    __HAL_TIM_SetCompare(&htim10, TIM_CHANNEL_1, float_constrain(float_rounding(imu_temp_pid.out), 0, UINT32_MAX));
}
