...
void OLED_ShowMessage(uint8_t page)
{
    ...
    case 0: 
        ...
        if(DevicesMonitor.Get_State(VISION_MONITOR) == Off_line)
        {
            oled_showstring(1, 1, (uint8_t *)"Vision");
        }
        if(DevicesMonitor.Get_State(COMMU_0X341_MONITOR)==Off_line/*  || DevicesMonitor.Get_State(COMMU_0X342_MONITOR)==Off_line */)
        {
            oled_showstring(1, 12, (uint8_t *)"CHAS CAN");
        }

        if(DevicesMonitor.Get_State(GIMBAL_YAW_MONITOR) == Off_line)
        {
            oled_showstring(2,1,(uint8_t *)"Yaw Motor");
        }
        if(DevicesMonitor.Get_State(GIMBAL_PIT_MONITOR) == Off_line)
        {
            oled_showstring(2,12,(uint8_t *)"Pit Motor");
        }

        if(DevicesMonitor.Get_State(FRIC_L_MONITOR) == Off_line)
        {
            oled_showstring(3, 1, (uint8_t *)"FricL");
        }
        if(DevicesMonitor.Get_State(FRIC_R_MONITOR) == Off_line)
        {
            oled_showstring(3, 7, (uint8_t *)"FricR");
        }
        if(DevicesMonitor.Get_State(RELOAD_MONITOR) == Off_line)
        {
    ...
    case 3:
        switch(Infantry.Get_VisionMode())
        {
        case Vision_Forcast:
