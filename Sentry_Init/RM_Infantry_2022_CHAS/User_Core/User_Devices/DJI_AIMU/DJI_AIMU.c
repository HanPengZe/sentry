...
static void mpu_offset_call(void)
{
  ...
  IMUwriteFlashData[0] = mpu_data.ax_offset;
  IMUwriteFlashData[1] = mpu_data.ay_offset;
  IMUwriteFlashData[2] = mpu_data.az_offset;
  IMUwriteFlashData[3] = mpu_data.gx_offset;
  IMUwriteFlashData[4] = mpu_data.gy_offset;
  IMUwriteFlashData[5] = mpu_data.gz_offset;
}
...
{
  mpu_data.ax_offset = (uint16_t)IMU_Offset[0];
  mpu_data.ay_offset = (uint16_t)IMU_Offset[1];
  mpu_data.az_offset = (uint16_t)IMU_Offset[2];
  mpu_data.gx_offset = (uint16_t)IMU_Offset[3];
  mpu_data.gy_offset = (uint16_t)IMU_Offset[4];
  mpu_data.gz_offset = (uint16_t)IMU_Offset[5];
}
...
static uint8_t mpu_device_init(void)
{
...
#if Calibrate == AIMU_Calibrate_On
  ...
  mpu_offset_call();
#elif Calibrate == AIMU_Calibrate_Off
...
void mpu_get_data(void)
{
  mpu_data.InfoUpdateFlag = mpu_read_bytes(MPU6500_ACCEL_XOUT_H, mpu_buff, 14);
...
/**
  ...
  * @usage  call in main() function
	*/
float totalYaw_LpfAttFactor = 1.0;
float gz_LpfAttFactor = 0.1;
void HAL_imu_attitude_update(void)
{
  /* yaw    -pi----pi */
  imu.yaw = -atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 57.3 + 180.0f;
  /* pitch  -pi/2----pi/2 */
  imu.pit = -asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3 + 180.0f;
  /* roll   -pi----pi  */
  imu.rol = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.3 + 180.0f;

  if (imu.yaw - imu.Previous_Yaw < -300)
  { //经过跳变边沿。
    imu.TurnsNum_Yaw++;
  }
  if (imu.Previous_Yaw - imu.yaw < -300)
  {
    imu.TurnsNum_Yaw--;
  }
  imu.Total_Yaw = imu.yaw + (360 * imu.TurnsNum_Yaw);

  imu.Previous_Yaw = imu.yaw;

  if (imu.pit - imu.Previous_Pitch < -300)
  { //经过跳变边沿。
    imu.TurnsNum_Pitch++;
  }
  if (imu.Previous_Pitch - imu.pit < -300)
  {
    imu.TurnsNum_Pitch--;
  }
  imu.Total_Pitch = imu.pit + (360 * imu.TurnsNum_Pitch);

  imu.Previous_Pitch = imu.pit;

  imu_Export.gz = (float)mpu_data.gz;
  imu_Export.gy = (float)mpu_data.gy;

  imu_Export.Total_Yaw = imu.Total_Yaw;
  imu_Export.Total_Pit = imu.Total_Pitch;


  // Filter_IIRLPF(&imu_Export.gz, &imu_Export.Finally_gz, gz_LpfAttFactor);
}

void IMU_data_read_over(mpu6500_real_data_t *mpu6500_real_data, ist8310_real_data_t *ist8310_real_data)
{

  mpu6500_real_data->accel[0] = mpu_data.ax * 0.002392578125f;
  mpu6500_real_data->accel[1] = mpu_data.ay * 0.002392578125f;
  mpu6500_real_data->accel[2] = mpu_data.az * 0.002392578125f;

  mpu6500_real_data->temp = 21 + mpu_data.temp / 333.87f;

  mpu6500_real_data->gyro[0] = mpu_data.gx * 0.00106526443603169529841533860381f;
  mpu6500_real_data->gyro[1] = mpu_data.gy * 0.00106526443603169529841533860381f;
  mpu6500_real_data->gyro[2] = mpu_data.gz * 0.00106526443603169529841533860381f;
  ...
  mpu6500_real_data->InfoUpdateFlag = mpu_data.InfoUpdateFlag;
